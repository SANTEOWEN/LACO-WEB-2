...
function handleSubjectFilterClick() {
  // Serialize form data
  var formData = new FormData(document.getElementById("subjectFilterForm"));

  // Create a new URLSearchParams object and append form data
  var params = new URLSearchParams(formData);

  // Build the URL for the filter_subject.php script with the serialized form data
  var url = "filter_subject.php?" + params.toString();
  ...
  fetch(url)
    ...
    .then((data) => {
      // Update the results table with the filtered data
      document.getElementById("subjectFilteredResults").innerHTML = data;
    })
    .catch((error) => console.error(error));
}